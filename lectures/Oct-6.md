...
In general, there are at most \frac{n}{2^{h+1}} nodes at a given
height h.
...
recall formula A.8: 
   ...
   \sum_{k=0}^{\infty} k x^k = \frac{x}{(1 - x)²}, for |x| < 1

   let x = 1/2:

   \sum_{k=0}^{\infty} \frac{k}{2^k} = \frac{1/2}{(1 - (1/2))²}
    = \frac{1/2}{1 \times 1 - 2\times 1/2 + 1/4}
    = \frac{1/2}{1/4} = 2

so we get

\sum_{h=0}^{log(n)} \frac{h}{2^h}  < 2

Thus O\left( n \sum_{h=0}^{log(n)} \frac{h}{2^h} \right)
= O( n \times 2 )
= O(n).
